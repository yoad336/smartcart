...
/* Shared client-side password policy. The authoritative minimum is set in
   Supabase → Authentication; this keeps the UI in sync and gives fast feedback. */
...
export function AuthProvider({ children }) {
  ...
  // True while the user arrived via a "reset password" email link and still
  // needs to choose a new password (Supabase fires a PASSWORD_RECOVERY event).
  ...
  useEffect(() => {
    ...
    // 2. Keep state in sync with login / logout / token refresh / recovery
  ...
  // Send a password-reset email. The link returns the user to this app, where
  // the PASSWORD_RECOVERY flow lets them set a new password.
  ...
  // Verify the user's current password (used before an in-app password change)
  // by re-authenticating with it. Returns { error } if it is wrong.
  async function verifyCurrentPassword(currentPassword) {
    if (!user?.email) return { error: { message: 'no-email' } }
...
const DEFAULT_PREFS = {
  ...
  maxDistanceKm: 15,
  // --- optimization layer ---
  fuelPricePerLiter: 7.4, // ₪ per liter (editable estimate)
  fuelEfficiencyKmPerLiter: 13, // car average km per liter
  allowSplit: true, // willing to split the shop between stores
  minSplitSavings: 15, // ₪ minimum basket saving that justifies a split
  optimizationMode: 'balanced', // 'max_savings' | 'balanced' | 'min_time' | 'convenience'
}

// The four optimization profiles shown on the preferences screen. Reused for
// labels elsewhere (e.g. the profile summary), so kept at module scope.
const OPT_MODES = [
  { key: 'max_savings', title: 'חיסכון מקסימלי', desc: 'המחיר הנמוך ביותר, גם אם רחוק יותר או דורש פיצול' },
  { key: 'balanced', title: 'מאוזן', desc: 'איזון בין מחיר, מרחק, זמן ונוחות' },
  { key: 'min_time', title: 'מינימום זמן', desc: 'הכי קרוב ומהיר, גם אם לא הכי זול' },
  { key: 'convenience', title: 'נוחות · סופר אחד', desc: 'קנייה אחת קצרה עם דירוג וחניה, ללא פיצול' },
]
const OPT_MODE_LABEL = OPT_MODES.reduce((m, o) => ((m[o.key] = o.title), m), {})
...
export function CartProvider({ children }) {
  ...
  const [preferences, setPreferences] = useState(() => ({ ...DEFAULT_PREFS, ...load(PREFS_KEY, {}) }))
  ...
  return (
    <nav className="bottom-nav" aria-label="ניווט ראשי">
      <div className="bottom-nav__brand">
        <span className="bottom-nav__brand-mark"><CartIcon size={20} /></span>
        <span className="bottom-nav__brand-text">SMART<span>CART</span></span>
      </div>
  ...
  async function handleSubmit(e) {
    ...
    if (isForgot) {
      ...
      try {
        ...
        setInfo('שלחנו קישור לאיפוס סיסמה לאימייל שלך. בדקו את תיבת הדואר (וגם ספאם) ולחצו על הקישור כדי לבחור סיסמה חדשה.')
      } catch (err) {
    ...
    if (isRegister && form.password.length < MIN_PASSWORD) {
      ...
      <div className="card">
        {isForgot ? (
          <div className="row gap-2 mb-3">
            <button className="header__back" onClick={() => switchMode('login')} type="button" aria-label="חזרה לכניסה">
              <BackIcon />
            </button>
            <div>
              <div className="label" style={{ fontWeight: 700 }}>איפוס סיסמה</div>
              <div className="small muted">נשלח אליך קישור לבחירת סיסמה חדשה</div>
            </div>
          </div>
        ) : (
          <div className="tabs">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {!isForgot && (
            <div className="field">
              ...
              <input
                ...
                type="password"
                className="input"
                placeholder={isRegister ? `לפחות ${MIN_PASSWORD} תווים` : 'הסיסמה שלך'}
                value={form.password}
          ...
          <button className="btn btn--primary" type="submit" disabled={busy}>
            {busy ? 'רגע…' : isForgot ? 'שליחת קישור איפוס' : isRegister ? 'יצירת חשבון' : 'כניסה'}
          </button>
        </form>
...
/* Shared validation for a new password + its confirmation. Returns an error
   string, or null when the pair is valid. */
function validateNewPassword(pw, confirmPw) {
  if (!pw || !confirmPw) return 'יש למלא את שני שדות הסיסמה'
  if (pw.length < MIN_PASSWORD) return `הסיסמה צריכה להכיל לפחות ${MIN_PASSWORD} תווים`
...
/* Full-screen takeover (wired in <App />) shown when the user returns from a
   "reset password" email link — Supabase emits a PASSWORD_RECOVERY session and
   we let them choose a new password without needing the old one. */
function ResetPassword() {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    setError('')
    const msg = validateNewPassword(pw, confirmPw)
    if (msg) {
      setError(msg)
      return
    }
  ...
  return (
    <div className="page page--centered">
      <div className="brand">
        <div className="brand__mark"><CartIcon size={28} /></div>
      </div>
      <div className="brand__name text-center">SMART<span>CART</span></div>
      <p className="muted text-center mb-4">בחירת סיסמה חדשה</p>

      <div className="card">
        {done ? (
          <>
            <div className="alert alert--success">הסיסמה עודכנה בהצלחה 🎉</div>
            <button className="btn btn--primary" onClick={finish}>כניסה לאפליקציה</button>
          </>
        ) : (
          <>
            <div className="row gap-2 mb-3">
              ...
              <div>
                <div className="label" style={{ fontWeight: 700 }}>איפוס סיסמה</div>
                <div className="small muted">בחרו סיסמה חדשה לחשבון</div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="field">
                <label className="field__label" htmlFor="newPw">סיסמה חדשה</label>
                <input
                  id="newPw"
                  type="password"
                  className="input"
              ...
              <div className="field">
                <label className="field__label" htmlFor="confirmPw">אימות סיסמה</label>
                <input
                  id="confirmPw"
                  type="password"
                  className="input"
            ...
            <button className="btn btn--ghost mt-3" type="button" onClick={cancel}>
              ביטול
            </button>
          </>
        )}
...
function AreaPreferences() {
  ...
  // Pull saved city/distance from the DB (if any) and merge over local prefs.
  // The optimization fields (fuel, split, mode) live in localStorage so the
  // app keeps working even if those columns do not exist in Supabase yet.
  useEffect(() => {
    if (!user) return
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Missing DB columns should never break the flow – prefs persist locally.
      setToast({ type: 'success', text: 'ההעדפות נשמרו במכשיר' })
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="page">
      <Header title="אזור והעדפות" subtitle="כדי שנחשב עבורך את הכדאיות האמיתית" showBack />

      {/* location & radius */}
      <div className="card">
        <div className="row gap-2 mb-3">
          <PinIcon size={18} className="text-primary" />
          <span className="label" style={{ fontWeight: 700 }}>מיקום ומרחק</span>
        </div>

        <div className="field">
          <label className="field__label" htmlFor="city">עיר</label>
          <select id="city" className="select" value={draft.city} onChange={(e) => set('city', e.target.value)}>
            {CITIES.map((c) => (
        ...
        <div className="field" style={{ marginBottom: 0 }}>
          <div className="row row--between">
            <label className="field__label" htmlFor="distance" style={{ marginBottom: 0 }}>רדיוס נסיעה מרבי</label>
          ...
          />
          <div className="row row--between small muted"><span>1 ק״מ</span><span>30 ק״מ</span></div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="row gap-2 mb-2">
          ...
          <span className="label" style={{ fontWeight: 700 }}>רכב ודלק</span>
        </div>
        <p className="small muted" style={{ marginBottom: 12 }}>
          משמש לחישוב עלות הדלק לנסיעה הלוך-חזור לכל סופר.
        </p>
        <div className="row gap-3">
          <div className="field grow" style={{ marginBottom: 0 }}>
            <label className="field__label" htmlFor="fuelPrice">מחיר דלק (₪ לליטר)</label>
            <input
              id="fuelPrice"
              type="number"
              inputMode="decimal"
              ...
              onChange={(e) => set('fuelPricePerLiter', Number(e.target.value))}
            />
          </div>
          <div className="field grow" style={{ marginBottom: 0 }}>
            <label className="field__label" htmlFor="fuelEff">צריכה (ק״מ לליטר)</label>
            <input
              id="fuelEff"
              type="number"
              inputMode="decimal"
              ...
              className="input"
              dir="ltr"
              value={draft.fuelEfficiencyKmPerLiter}
              onChange={(e) => set('fuelEfficiencyKmPerLiter', Number(e.target.value))}
            />
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="row gap-2 mb-3">
          ...
          <span className="label" style={{ fontWeight: 700 }}>פיצול קנייה בין סופרים</span>
        </div>

        <div className="field">
          <label className="field__label">מוכן/ה לפצל קנייה בין כמה סופרים?</label>
          <div className="chip-row">
            <button
              type="button"
              className={`chip ${draft.allowSplit ? 'chip--active' : ''}`}
              onClick={() => set('allowSplit', true)}
            >
              כן, לחיסכון
            </button>
            <button
        ...
        <div className="field" style={{ marginBottom: 0, opacity: draft.allowSplit ? 1 : 0.5 }}>
          <div className="row row--between">
            <label className="field__label" htmlFor="minSplit" style={{ marginBottom: 0 }}>חיסכון מינימלי שמצדיק פיצול</label>
            <span className="badge badge--primary">{formatILS(draft.minSplitSavings)}</span>
          </div>
          <input
            id="minSplit"
            type="range"
            ...
            max="60"
            step="5"
            value={draft.minSplitSavings}
            disabled={!draft.allowSplit}
            onChange={(e) => set('minSplitSavings', Number(e.target.value))}
            style={{ width: '100%', marginTop: 12, accentColor: 'var(--color-primary)' }}
          />
          <div className="row row--between small muted"><span>₪0</span><span>₪60</span></div>
        </div>
      </div>

      {/* optimization mode */}
      <h2 className="h3 mt-6 mb-3">מצב אופטימיזציה</h2>
      <div className="col gap-3">
        {OPT_MODES.map((m) => (
          <ModeCard
            key={m.key}
            active={draft.optimizationMode === m.key}
            onClick={() => set('optimizationMode', m.key)}
            Icon={m.key === 'max_savings' ? TagIcon : m.key === 'min_time' ? PinIcon : m.key === 'convenience' ? CartIcon : SparkIcon}
            title={m.title}
            desc={m.desc}
          />
        ))}
      ...
      <div className="col gap-3 mt-6">
        ...
        <button className="btn btn--ghost" onClick={() => navigate('/compare')}>להשוואת מחירים</button>
      </div>
    </div>
  )
}
...
function PriceComparison() {
  ...
  const result = useMemo(() => {
    ...
    return recommendOptimized(items, catalog.supermarkets, catalog.prices, preferences)
    ...
    return (
      <div className="page">
        ...
        <div className="empty">
          ...
          <button className="btn btn--primary mt-4" onClick={() => navigate('/create')}>יצירת רשימה</button>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="page">
        ...
        <div className="center-screen">
          <div>
            <div className="spinner" />
            <p className="muted mt-3">מחשבים כדאיות…</p>
          </div>
        </div>
      </div>
    )
  ...
  if (result.outOfRange || !result.evaluations.length) {
    return (
      <div className="page">
        <Header title="השוואת מחירים" subtitle={`${preferences.city} · עד ${preferences.maxDistanceKm} ק״מ`} showBack />
        {notice && <div className="alert alert--info">{notice}</div>}
        <div className="empty">
          <div className="empty__emoji">📍</div>
          <p>אין סופרים בטווח של {preferences.maxDistanceKm} ק״מ. הגדילו את רדיוס הנסיעה בהעדפות.</p>
          <button className="btn btn--primary mt-4" onClick={() => navigate('/preferences')}>פתיחת ההעדפות</button>
        </div>
      </div>
    )
  }

  const stores = result.evaluations
  const maxEffective = Math.max(...stores.map((s) => s.effectiveTotal)) || 1
  const best = result.recommended
  const { cheapestId, closestId, bestValueId, mostConvenientId } = result.tags

  return (
    <div className="page">
      <Header
        title="השוואת מחירים"
        subtitle={`${preferences.city} · עד ${preferences.maxDistanceKm} ק״מ · ${OPT_MODE_LABEL[preferences.optimizationMode]}`}
        showBack
      />

      {notice && <div className="alert alert--info">{notice}</div>}

      {/* headline: best value after fuel */}
      <div className="card" style={{ background: 'var(--color-secondary)', color: '#fff', border: 'none' }}>
        <div className="small" style={{ opacity: 0.9 }}>הכי משתלם בפועל (אחרי דלק)</div>
        <div className="row row--between mt-2">
          <div className="h2" style={{ color: '#fff' }}>{best.store.name}</div>
          <div className="score" style={{ background: 'rgba(255,255,255,.2)', color: '#fff' }}>
            <span className="score__num">{best.worthiness}</span><span className="score__max">/100</span>
          </div>
        </div>
        <div className="small mt-2" style={{ opacity: 0.95 }}>
          עלות כוללת {formatILS(best.effectiveTotal)} · חיסכון {formatILS(result.savingsVsAvg)} מתחת לממוצע
        </div>
      </div>

      <h2 className="h3 mt-6 mb-3">לפי כדאיות כוללת</h2>

      <div className="col gap-3">
        {stores.map((s) => {
          const isBest = s.store.id === bestValueId
          const salesCount = s.lines.filter((l) => l.onSale).length
          const tier = s.worthiness >= 75 ? 'high' : s.worthiness >= 50 ? 'mid' : 'low'
          return (
            <div key={s.store.id} className={`store-card ${isBest ? 'store-card--best' : ''}`}>
              <div className="row row--between">
                <div className="row gap-3">
                  <span className="store-logo" style={{ background: s.store.color }}>{s.store.name.charAt(0)}</span>
                  <div>
                    <div className="label" style={{ fontWeight: 700 }}>{s.store.name}</div>
                    <div className="small muted row gap-2">
                      <PinIcon size={13} /> {s.distanceKm} ק״מ · {s.travelMinutes} דק׳ · ⭐ {s.rating}
                    </div>
                  </div>
                </div>
                <div className={`score score--${tier}`}>
                  <span className="score__num">{s.worthiness}</span><span className="score__max">/100</span>
                </div>
              </div>

              {/* recommendation tags */}
              <div className="row gap-2 mt-3" style={{ flexWrap: 'wrap' }}>
                {s.store.id === bestValueId && <span className="badge badge--best"><CheckIcon size={13} /> הכי משתלם בפועל</span>}
                {s.store.id === cheapestId && <span className="badge badge--primary"><TagIcon size={13} /> הכי זול</span>}
                {s.store.id === closestId && <span className="badge badge--accent"><PinIcon size={13} /> הכי קרוב</span>}
                {s.store.id === mostConvenientId && <span className="badge badge--violet"><StarIcon size={13} /> הכי נוח</span>}
              </div>

              {/* metrics */}
              <div className="metrics">
                <div className="metric"><span className="metric__label">מחיר מוצרים</span><span className="metric__value">{formatILS(s.productsTotal)}</span></div>
                <div className="metric"><span className="metric__label">עלות דלק</span><span className="metric__value">{formatILS(s.fuelCost)}</span></div>
                <div className="metric metric--accent"><span className="metric__label">עלות כוללת</span><span className="metric__value">{formatILS(s.effectiveTotal)}</span></div>
                <div className="metric"><span className="metric__label">זמן נסיעה</span><span className="metric__value">{s.travelMinutes} דק׳</span></div>
              </div>

              {/* effective-cost bar (shorter = cheaper) */}
              <div className="bar mt-3">
                <div
                  className="bar__fill"
                  style={{ width: `${Math.max(8, (s.effectiveTotal / maxEffective) * 100)}%`, background: isBest ? 'var(--color-secondary)' : 'var(--color-primary)' }}
                />
              </div>

              <div className="row gap-2 mt-3" style={{ flexWrap: 'wrap' }}>
                {salesCount > 0 && <span className="badge badge--sale"><TagIcon size={13} /> {salesCount} מבצעים</span>}
                {s.parking && <span className="badge badge--muted">חניה</span>}
                {s.missingCount > 0
                  ? <span className="badge badge--muted">חסרים {s.missingCount} מוצרים</span>
                  : <span className="badge badge--primary">כל המוצרים</span>}
              </div>
            </div>
          )
        })}
      ...
      <button className="btn btn--primary mt-6" onClick={() => navigate('/result')}>
        <CartIcon size={18} /> לעגלה המשתלמת ביותר
      </button>
    </div>
  )
}
...
function SmartCartResult() {
  ...
  if (!items.length) {
    return (
      <div className="page">
        <Header title="העגלה המשתלמת" showBack />
        <div className="empty">
          <div className="empty__emoji">✨</div>
          <p>אין עדיין עגלה. צרו רשימה ונחשב עבורכם את הסל המשתלם ביותר.</p>
          <button className="btn btn--primary mt-4" onClick={() => navigate('/create')}>יצירת רשימה</button>
        </div>
      </div>
    )
  ...
  if (result.outOfRange || !result.recommended) {
    return (
      <div className="page">
        <Header title="העגלה המשתלמת" showBack />
        <div className="empty">
          <div className="empty__emoji">📍</div>
          <p>אין סופרים בטווח שהגדרת. הגדילו את רדיוס הנסיעה בהעדפות.</p>
          <button className="btn btn--primary mt-4" onClick={() => navigate('/preferences')}>פתיחת ההעדפות</button>
        </div>
      </div>
    )
  ...
  return (
    <div className="page">
      <Header title="העגלה המשתלמת ביותר" subtitle={listName} showBack />

      {/* hero: best value in practice */}
      <div className="card" style={{ background: 'var(--color-primary)', color: '#fff', border: 'none' }}>
        <div className="row row--between">
          <div className="row gap-2">
            <SparkIcon size={18} />
            <span className="small" style={{ opacity: 0.9 }}>הסל המשתלם ביותר בפועל</span>
          </div>
          <div className="score" style={{ background: 'rgba(255,255,255,.2)', color: '#fff' }}>
            <span className="score__num">{rec.worthiness}</span><span className="score__max">/100</span>
          </div>
        </div>
        <div className="h1 mt-2" style={{ color: '#fff' }}>{formatILS(rec.effectiveTotal)}</div>
        <div className="row gap-2 small" style={{ opacity: 0.95, flexWrap: 'wrap' }}>
          <span className="row gap-1"><CartIcon size={15} /> {rec.store.name}</span>
          <span className="row gap-1"><PinIcon size={13} /> {rec.distanceKm} ק״מ · {rec.travelMinutes} דק׳</span>
        </div>
      </div>

      {/* breakdown */}
      <div className="metrics" style={{ marginTop: 'var(--sp-4)' }}>
        <div className="metric"><span className="metric__label">מחיר הסל (לפני דלק)</span><span className="metric__value">{formatILS(rec.productsTotal)}</span></div>
        <div className="metric"><span className="metric__label">עלות דלק משוערת</span><span className="metric__value">{formatILS(rec.fuelCost)}</span></div>
        <div className="metric metric--accent"><span className="metric__label">עלות כוללת בפועל</span><span className="metric__value">{formatILS(rec.effectiveTotal)}</span></div>
        <div className="metric"><span className="metric__label">חיסכון מול הממוצע</span><span className="metric__value">{formatILS(result.savingsVsAvg)}</span></div>
      </div>

      {/* why this recommendation */}
      <h2 className="h3 mt-6 mb-3">למה זו ההמלצה?</h2>
      <div className="why">{result.explanation}</div>

      {/* split recommendation */}
      <h2 className="h3 mt-6 mb-3">פיצול קנייה</h2>
      <div className="card">
        <div className="row gap-2 mb-2">
          <SparkIcon size={16} className="text-primary" />
          <span className="label" style={{ fontWeight: 700 }}>
            {split && split.worthwhile ? 'מומלץ לפצל את הקנייה' : 'מומלץ לקנות בסופר אחד'}
          </span>
        </div>
        <p className="small" style={{ color: 'var(--color-text)' }}>{split ? split.message : ''}</p>
        {split && split.considered && split.storesUsedCount > 1 && (
          <div className="metrics" style={{ marginTop: 'var(--sp-3)' }}>
            <div className="metric"><span className="metric__label">סל בפיצול</span><span className="metric__value">{formatILS(split.productsTotal)}</span></div>
            <div className="metric"><span className="metric__label">חוסך מהסל</span><span className="metric__value">{formatILS(split.grossSavings)}</span></div>
            <div className="metric"><span className="metric__label">תוספת זמן</span><span className="metric__value">{split.addedMinutes} דק׳</span></div>
            <div className="metric"><span className="metric__label">חנויות בפיצול</span><span className="metric__value">{split.storesUsedCount}</span></div>
          </div>
        )}
      </div>

      {/* itemized cart at the recommended store */}
      <h2 className="h3 mt-6 mb-3">פירוט העגלה ב{rec.store.name}</h2>
      <ul>
        {rec.lines.map((l) => (
          <li className="list-item" key={l.product_id}>
            <span className="product-emoji">{l.emoji || '🛒'}</span>
            <span className="grow">
              <span className="label" style={{ display: 'block', fontWeight: 700 }}>{l.name}</span>
            ...
            <span className="label" style={{ fontWeight: 700 }}>
              {l.available ? formatILS(l.lineTotal) : <span className="small text-error">לא זמין</span>}
            </span>
          </li>
        ))}
      </ul>
      ...
      {result.substitutions.length > 0 && (
        <>
          <h2 className="h3 mt-6 mb-3">המלצות תחליפים חכמות</h2>
          <div className="col gap-3">
            {result.substitutions.map((sub) => (
              <div className="card row gap-3" key={sub.fromId} style={{ borderColor: 'var(--color-secondary)' }}>
                <span className="product-emoji">{sub.toEmoji}</span>
                <div className="grow">
                  <div className="label" style={{ fontWeight: 700 }}>החלפת {sub.fromName}</div>
                  <div className="small muted">ל{sub.toName}{sub.note ? ` · ${sub.note}` : ''}</div>
                </div>
                <span className="badge badge--best">−{formatILS(sub.saving)}</span>
              </div>
            ))}
          </div>
        </>
      ...
      {result.alternatives.length > 0 && (
        <>
          <h2 className="h3 mt-6 mb-3">השוואת חלופות</h2>
          <div className="col gap-3">
            {result.alternatives.map((a) => (
              <div className="store-card" key={a.store.id}>
                <div className="row row--between">
                  <div className="row gap-3">
                    <span className="store-logo" style={{ background: a.store.color }}>{a.store.name.charAt(0)}</span>
                    <div>
                      <div className="label" style={{ fontWeight: 700 }}>{a.store.name}</div>
                      <div className="small muted row gap-2"><PinIcon size={13} /> {a.distanceKm} ק״מ · {a.travelMinutes} דק׳</div>
                    </div>
                  </div>
                  <div className={`score score--${a.worthiness >= 75 ? 'high' : a.worthiness >= 50 ? 'mid' : 'low'}`}>
                    <span className="score__num">{a.worthiness}</span><span className="score__max">/100</span>
                  </div>
                </div>
                <div className="metrics">
                  <div className="metric"><span className="metric__label">מחיר מוצרים</span><span className="metric__value">{formatILS(a.productsTotal)}</span></div>
                  <div className="metric"><span className="metric__label">עלות דלק</span><span className="metric__value">{formatILS(a.fuelCost)}</span></div>
                  <div className="metric metric--accent"><span className="metric__label">עלות כוללת</span><span className="metric__value">{formatILS(a.effectiveTotal)}</span></div>
                  <div className="metric"><span className="metric__label">דירוג</span><span className="metric__value">⭐ {a.rating}</span></div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="col gap-3 mt-6">
        <button className="btn btn--ghost" onClick={() => navigate('/compare')}>חזרה להשוואה</button>
        <button className="btn btn--ghost" onClick={() => navigate('/preferences')}>שינוי העדפות</button>
        <button className="btn btn--ghost" onClick={() => navigate('/create')}>עריכת הרשימה</button>
      </div>
    </div>
  )
}
    ...
    <div className="page">
      ...
      <div className="col gap-3">
        <button className="action-tile" onClick={() => navigate('/settings')}>
          ...
          <span className="grow">
            <span className="label" style={{ display: 'block', fontWeight: 700 }}>הגדרות חשבון ואבטחה</span>
            <span className="small muted">פרטי החשבון והחלפת סיסמה</span>
          </span>
        </button>
        ...
        <button className="action-tile" onClick={() => navigate('/preferences')}>
          ...
          <span className="grow">
            ...
            <span className="small muted">
              ...
              {OPT_MODE_LABEL[preferences.optimizationMode] || 'מאוזן'}
            </span>
          </span>
        </button>
...
/* Basic account settings, reachable from the personal area (Profile → "הגדרות
   חשבון ואבטחה"). Includes an in-app password change that first re-verifies the
   current password before setting the new one. */
function Settings() {
  const navigate = useNavigate()
  const { user, signOut, updatePassword, verifyCurrentPassword } = useAuth()
  ...
  const email = user?.email || ''
  const fullName = user?.user_metadata?.full_name || email.split('@')[0] || 'משתמש'
  const initials = fullName.trim().charAt(0).toUpperCase() || '🙂'
  ...
  async function handleChangePassword(e) {
    ...
    if (!current) {
      setToast({ type: 'error', text: 'יש להזין את הסיסמה הנוכחית' })
      return
    }
    const msg = validateNewPassword(pw, confirmPw)
    if (msg) {
    ...
    if (pw === current) {
      setToast({ type: 'error', text: 'הסיסמה החדשה זהה לנוכחית' })
      return
    }

    setBusy(true)
    try {
      // 1) re-verify the current password before allowing a change
      const { error: vErr } = await verifyCurrentPassword(current)
      if (vErr) {
        setToast({ type: 'error', text: 'הסיסמה הנוכחית שגויה' })
        return
      }
      // 2) set the new password
      const { error } = await updatePassword(pw)
      if (error) throw error
      setToast({ type: 'success', text: 'הסיסמה הוחלפה בהצלחה' })
      setCurrent('')
      setPw('')
      setConfirmPw('')
    } catch (err) {
      setToast({ type: 'error', text: translateAuthError(err.message) })
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <div className="page">
      <Header title="הגדרות חשבון ואבטחה" subtitle="פרטי החשבון, אבטחה והעדפות" showBack />
      ...
      <div className="card row gap-3">
        ...
        <div className="grow">
          <div className="h3" style={{ marginBottom: 2 }}>{fullName}</div>
          <div className="small muted" dir="ltr" style={{ textAlign: 'start' }}>{email || 'לא מחובר'}</div>
        </div>
      </div>

      {!isSupabaseConfigured && (
        <div className="alert alert--info mt-4">
          החלפת סיסמה דורשת חיבור ל-Supabase. האפליקציה פועלת כעת במצב דמו (ראו README).
        </div>
      )}

      {/* change password */}
      <h2 className="h3 mt-6 mb-3">החלפת סיסמה</h2>
      <div className="card">
        {toast && <div className={`alert alert--${toast.type}`}>{toast.text}</div>}

        <form onSubmit={handleChangePassword}>
          <div className="field">
            <label className="field__label" htmlFor="currentPw">סיסמה נוכחית</label>
            <input
              id="currentPw"
              type="password"
              className="input"
          ...
          <div className="field">
            <label className="field__label" htmlFor="newPwSet">סיסמה חדשה</label>
            <input
              id="newPwSet"
              type="password"
              className="input"
          ...
          <div className="field">
            <label className="field__label" htmlFor="confirmPwSet">אימות סיסמה חדשה</label>
            <input
              id="confirmPwSet"
              type="password"
              className="input"
      ...
      {/* other settings */}
      <h2 className="h3 mt-6 mb-3">העדפות</h2>
      <button className="action-tile" onClick={() => navigate('/preferences')}>
        <span className="action-tile__icon" style={{ background: 'var(--color-secondary)' }}>
          <PinIcon size={20} />
        </span>
        <span className="grow">
          <span className="label" style={{ display: 'block', fontWeight: 700 }}>אזור והעדפות קנייה</span>
          <span className="small muted">עיר, מרחק מרבי ושיטת קנייה</span>
        </span>
      </button>
...
function App() {
  ...
  // When the user arrives from a password-reset email link, take over the UI
  // with the "choose a new password" screen until they finish (or cancel).
  if (passwordRecovery) {
    return (

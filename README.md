...
🔗 **אתר חי (Vercel):** _[הדביקו כאן את הקישור אחרי הפריסה]_
💻 **קוד מקור (GitHub):** _[הדביקו כאן את קישור הריפו]_
...
- "סל חכם" — המלצה על החנות הזולה ביותר וחישוב החיסכון.
- מצב פיצול בין חנויות לחיסכון מרבי.
- שמירת רשימות, אזור והעדפות (עיר, רדיוס, מצב חיסכון).
...
| `support.js` | **לוגיקה טהורה + שכבת נתונים**: מנוע השוואת המחירים והעגלה (פונקציות נטו, ללא React), ושירות הנתונים שמדבר עם Supabase — עם נפילה אוטומטית לנתוני דמו. |
| `supabaseClient.js` | יצירת ה־Supabase client ממשתני הסביבה, עם בדיקת תקינות (`isSupabaseConfigured`). |
| `data.js` | קטלוג דמו (5 רשתות, 17 מוצרים, מחירים) — מאפשר ריצה מלאה ללא שרת. |
...
הדיאגרמה מתארת את הסכימה: 7 טבלאות + `auth.users` המנוהל ע"י Supabase.
...
- האפליקציה כוללת קטלוג דמה (5 רשתות, 17 מוצרים, מחירים) — הזרימה המרכזית עובדת מקצה לקצה גם בלי להזין נתונים.
